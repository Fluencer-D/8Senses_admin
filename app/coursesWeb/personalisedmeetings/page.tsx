...
const PLAN_OPTIONS = ["prime", "basic", "free"] // Example plans
...
export default function MeetingAdminPanel() {
  ...
  const API_BASE_URL = "http://localhost:5000/api/meetings" // Assuming Express server runs on port 5000
  ...
  const createMeeting = async () => {
    try {
      const response = await fetch(API_BASE_URL, {
        ...
        body: JSON.stringify({
          ...formData,
          associatedPlans: formData.associatedPlans.filter((plan) => plan.trim() !== ""),
        }),
      })
      const data = await response.json()
      if (data.success) {
  ...
  const handleEdit = (meeting: Meeting) => {
    ...
    setFormData({
      ...
      associatedPlans: meeting.associatedPlans.length > 0 ? meeting.associatedPlans : [],
    })
    setCurrentView("edit")
  }
  ...
  const updateAssociatedPlanField = (index: number, value: string) => {
    setFormData((prev) => ({
      ...prev,
      associatedPlans: prev.associatedPlans.map((item, i) => (i === index ? value : item)),
    }))
  }
  ...
  const filteredMeetings = useMemo(() => {
    return meetings.filter((meeting) => {
      ...
      const matchesPlan = selectedPlanFilter === "" || meeting.associatedPlans.includes(selectedPlanFilter)

      const { status: meetingStatus } = getMeetingStatus(meeting)
      const matchesStatus = selectedStatusFilter === "" || meetingStatus === selectedStatusFilter
  ...
  const upcomingMeetings = meetings.filter((m) => getMeetingStatus(m).status === "Upcoming").length
  const ongoingMeetings = meetings.filter((m) => getMeetingStatus(m).status === "Ongoing").length
  const completedMeetings = meetings.filter((m) => getMeetingStatus(m).status === "Completed").length
      ...
      <div className="py-8">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold text-slate-900 mb-2">Meeting Management</h1>
              <p className="text-lg text-slate-600">Schedule and oversee your virtual consultations</p>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-slate-900">{totalMeetings}</p>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-blue-600">{upcomingMeetings}</p>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-green-600">{ongoingMeetings}</p>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-600">Unique Doctors</p>
                    <p className="text-3xl font-bold text-purple-600">{uniqueDoctors}</p>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-8 mb-8 border border-slate-200">
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1">
                  <label className="block text-sm font-semibold text-slate-700 mb-3">Search Meetings</label>
                ...
                <div className="lg:w-48">
                  <label className="block text-sm font-semibold text-slate-700 mb-3">Filter by Plan</label>
                  <select
                  ...
                  >
                    <div className="p-6">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <span className={`flex items-center gap-1 px-3 py-1 text-xs font-bold rounded-full ${color}`}>
                          {icon}
                          {status}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-slate-600 mb-2">
                        ...
                        <span className="text-sm font-medium">{meeting.hostDoctor}</span>
                      </div>
                      <div className="flex items-center gap-2 text-slate-600 mb-2">
                        <Calendar className="w-4 h-4" />
            ...
            {filteredMeetings.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-4">No meetings found</h3>
                <p className="text-lg text-slate-600">Try adjusting your search or create a new meeting.</p>
              </div>
            )}
          </div>
        )}
            ...
            <div className="px-8 py-6 border-b border-slate-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900">Meeting Details</h2>
                    <p className="text-slate-600 mt-1">Comprehensive view of the meeting information</p>
                  </div>
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">{selectedMeeting.hostDoctor}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-slate-900">Approx. Duration</p>
                      <p className="text-slate-600">{selectedMeeting.approxDuration}</p>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">
                    {currentView === "create" ? "Create New Meeting" : "Edit Meeting"}
                  </h2>
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-800 mb-2">Meeting Link</label>
                    <div className="relative">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-800 mb-2">Host Doctor</label>
                    <div className="relative">
                      <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 w-5 h-5" />
                      <input
                        type="text"
                        value={formData.hostDoctor}
                        onChange={(e) => setFormData((prev) => ({ ...prev, hostDoctor: e.target.value }))}
                        className="w-full pl-12 pr-4 py-3 text-lg border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200"
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-800 mb-2">Approx. Duration</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData((prev) => ({ ...prev, approxDuration: e.target.value }))}
                        className="w-full pl-12 pr-4 py-3 text-lg border-2 border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200"
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                    <Users className="w-5 h-5 text-slate-600" />
                    <h3 className="text-xl font-bold text-slate-900">Associated Plans</h3>
                  </div>
                  <p className="text-sm text-slate-600">
                    Select which user plans this meeting is available for. Leave empty for all plans.
                  </p>
                  <div className="space-y-3">
                    {formData.associatedPlans.map((plan, index) => (
                      <div key={index} className="flex gap-3 items-center">
                        <select
                          value={plan}
                          onChange={(e) => updateAssociatedPlanField(index, e.target.value)}
              ...
              <div className="mt-12 flex justify-end gap-4">
                ...
                <button
                  type="button"
                  onClick={currentView === "create" ? createMeeting : updateMeeting}
                ...
                >
                  ...
                  {currentView === "create" ? "Create Meeting" : "Update Meeting"}
                </button>
              </div>
            </div>
          </div>
