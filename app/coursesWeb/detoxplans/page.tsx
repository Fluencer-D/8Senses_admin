...
export default function DetoxAdminPanel() {
  ...
  const createDetoxPlan = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/detox/create-detox`, {
        ...
        body: JSON.stringify({
          ...formData,
          meals: formData.meals.filter((meal) => meal.day.trim() !== "" && meal.mealPlan.trim() !== ""),
        }),
      })
      const data = await response.json()
      if (data.success) {
  ...
  const handleEdit = (plan: DetoxPlan) => {
    ...
    setFormData({
      ...
      meals: plan.meals.length > 0 ? plan.meals : [{ day: "Day 1", mealPlan: "" }],
    })
    setCurrentView("edit")
  }
  ...
  const updateMealField = (index: number, field: "day" | "mealPlan", value: string) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const filteredDetoxPlans = detoxPlans.filter((plan) => {
    ...
    const matchesDuration = selectedDuration === "" || plan.duration === selectedDuration
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-6 text-xl font-semibold text-gray-800">Loading detox plans...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="py-8">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold text-slate-900 mb-2">Detox Plan Management</h1>
              <p className="text-lg text-slate-600">Curate and manage your healthy detox programs</p>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-600">Total Detox Plans</p>
                    <p className="text-3xl font-bold text-slate-900">{detoxPlans.length}</p>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-600">Shortest Duration</p>
                    <p className="text-3xl font-bold text-emerald-600">
                      {detoxPlans.length > 0
                        ? detoxPlans.reduce((min, p) => {
                            const minDays = Number.parseInt(min.split(" ")[0])
                            const pDays = Number.parseInt(p.duration.split(" ")[0])
              ...
              <div className="bg-white rounded-2xl shadow-sm p-6 border border-slate-200">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-600">Longest Duration</p>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-8 mb-8 border border-slate-200">
              <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1">
                  <label className="block text-sm font-semibold text-slate-700 mb-3">Search Plans</label>
                ...
                <div className="lg:w-64">
                  <label className="block text-sm font-semibold text-slate-700 mb-3">Filter by Duration</label>
                  <select
            ...
            {filteredDetoxPlans.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-4">No detox plans found</h3>
                <p className="text-lg text-slate-600">Try adjusting your search or create a new plan.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200">
            <div className="px-8 py-6 border-b border-slate-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold text-slate-900">Detox Plan Details</h2>
                    <p className="text-slate-600 mt-1">Comprehensive view of the detox program</p>
                  </div>
                </div>
                <button
            ...
            <div className="p-8">
              <div className="space-y-6">
                <div>
                  <h3 className="text-3xl font-bold text-slate-900 mb-2">{selectedDetoxPlan.title}</h3>
                  <p className="text-slate-600 leading-relaxed">{selectedDetoxPlan.description}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">{selectedDetoxPlan.duration}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">{selectedDetoxPlan.createdBy}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-slate-600">
                        {new Date(selectedDetoxPlan.createdAt).toLocaleDateString("en-US", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                        })}
                  ...
                  <div className="space-y-6">
                    {selectedDetoxPlan.meals.map((meal, index) => (
                      <div key={index} className="bg-slate-50 p-6 rounded-xl border border-slate-200">
                        ...
                        <p className="text-slate-700 leading-relaxed">{meal.mealPlan}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">
                    {currentView === "create" ? "Create New Detox Plan" : "Edit Detox Plan"}
                  </h2>
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-800 mb-2">Plan Title</label>
                    <input
                      type="text"
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
                    <Apple className="w-5 h-5 text-slate-600" />
                    <h3 className="text-xl font-bold text-slate-900">Daily Meal Plan</h3>
                  </div>
                    ...
                    {formData.meals.map((meal, index) => (
                      <div key={index} className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                        <div className="flex items-center justify-between mb-3">
                          <label className="block text-sm font-bold text-slate-800">Day {index + 1}</label>
                        ...
                        <div className="mb-3">
                          <label className="block text-xs font-semibold text-slate-700 mb-1">Day Label</label>
                          <input
                            type="text"
                            value={meal.day}
                            onChange={(e) => updateMealField(index, "day", e.target.value)}
                            className="w-full px-3 py-2 text-base border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200"
                        ...
                        <div>
                          ...
                          <textarea
                            value={meal.mealPlan}
                            onChange={(e) => updateMealField(index, "mealPlan", e.target.value)}
                            rows={4}
                            className="w-full px-3 py-2 text-base border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-200 resize-none"
              ...
              <div className="mt-12 flex justify-end gap-4">
                ...
                <button
                  type="button"
                  onClick={currentView === "create" ? createDetoxPlan : updateDetoxPlan}
