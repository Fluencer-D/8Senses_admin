    ...
    const fetchData = async () => {
      try {
        ...
        const validatedOrders = ordersData.map(order => ({
          ...
          user: order.user || { _id: 'unknown', email: 'unknown', firstName: 'Anonymous', lastName: 'User' },
        ...
        const totalSales = validatedOrders.reduce((sum, order) => sum + (order.total || 0), 0);
        
        // Count active orders safely
        const activeOrders = validatedOrders.filter(order => 
          order.status && ['pending', 'processing', 'shipped'].includes(order.status)
        ).length;
        ...
        <div className="flex-1 p-6 bg-gray-100 rounded-xl mt-10 ml-68">
          ...
          <div className="flex items-center justify-between flex-wrap gap-3 p-4 bg-gray-100 w-[105%] -ml-5">
            ...
            <div className="flex items-center space-x-4 mr-5">
              <button className="flex items-center px-3 py-1.5 border rounded-md text-gray-700">
                <svg
              ...
              <button className="px-4 py-1.5 bg-[#C83C92] text-white rounded-lg">
                + Add Product
              </button>
            </div>
          </div>
        ...
        <tbody className="divide-y divide-gray-200 text-gray-700">
          {orders.length > 0 ? (
            orders.map((order) => {
              ...
              return (
                <tr key={order._id} className="h-16 hover:bg-gray-50 transition">
                  <td className="px-6 py-4 font-medium">{order.orderNumber || 'N/A'}</td>
                  <td className="px-6 py-4 flex items-center space-x-3">
                    <Image 
                      src={mainItem?.image || '/placeholder-product.png'} 
                    ...
                    />
                    <div>
                      <span className="block">{mainItem?.name || 'Unnamed Product'}</span>
                      {additionalItems > 0 && (
                        <span className="text-gray-400 text-sm">+{additionalItems} other product{additionalItems > 1 ? 's' : ''}</span>
                      )}
                    </div>
                  </td>
