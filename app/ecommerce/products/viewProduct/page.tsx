    ...
    const fetchProductData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          ...
          {
            headers: {
              'Authorization': `Bearer ${token}` // Include only if the endpoint requires auth
            }
          }
        );
        const productData = response.data.data; // Access the data property in the response
        ...
        setCategoryId(productData.category?._id || "");  // Store category ID
        ...
        if (axios.isAxiosError(err)) {
          if (err.response?.status === 404) {
            setError("Product not found. It might have been deleted or never existed.");
            ...
            setError(`Error loading product: ${err.response.data.message || 'Unknown error'}`);
          }
        } else {
    ...
    return (
      <div className="p-6 flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <button onClick={() => router.push("/ecommerce/products")} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded-lg">
            Return to Products
          </button>
        </div>
      </div>
    ...
    <div className="p-6 max-w-[84%] mt-15 ml-70 mx-auto overflow-y-auto hide-scrollbar">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="text-gray-500 text-sm">
          <span className="text-blue-600 cursor-pointer" onClick={() => router.push("/ecommerce")}>E-commerce</span> &gt;{" "}
          <span className="text-blue-600 cursor-pointer" onClick={() => router.push("/ecommerce/products")}>Products</span> &gt;{" "}
        ...
        <div className="flex gap-2">
          ...
          {isEditing ? (
            <button onClick={handleUpdate} className="px-4 py-2 bg-[#C83C92] text-white rounded-lg flex items-center gap-1">
            ...
            <button onClick={toggleEditMode} className="px-4 py-2 bg-[#245BA7] text-white rounded-lg flex items-center gap-1">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M4.16667 15.8333H5.83333V14.1667H4.16667V15.8333ZM4.16667 10.8333H5.83333V9.16667H4.16667V10.8333ZM4.16667 5.83333H5.83333V4.16667H4.16667V5.83333ZM7.5 15.8333H15.8333V14.1667H7.5V15.8333ZM7.5 10.8333H15.8333V9.16667H7.5V10.8333ZM7.5 4.16667V5.83333H15.8333V4.16667H7.5Z" fill="white"/>
              </svg> Edit
            </button>
          )}
        </div>
        ...
        <div className="w-2/3 space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <input
              ...
              className={`w-full border border-[#E0E2E7] p-3 rounded-lg ${isEditing ? 'bg-white' : 'bg-[#F9F9FC]'} mb-4 text-[#333843]`}
              disabled={!isEditing}
            />
            ...
            <textarea
              ...
              className={`w-full border border-[#E0E2E7] p-3 rounded-lg ${isEditing ? 'bg-white' : 'bg-[#F9F9FC]'} h-24 text-[#333843]`}
              disabled={!isEditing}
            />
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="mb-4 border border-[#E0E2E7] rounded-lg p-2">
              <div className="bg-gray-100 h-40 w-full flex items-center justify-center rounded">
                <img 
                  src={image ? URL.createObjectURL(image) : "https://placehold.co/400x320"} 
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <input
                  ...
                  className={`w-full border border-[#E0E2E7] p-3 pl-8 rounded-lg ${isEditing ? 'bg-white' : 'bg-[#F9F9FC]'} text-[#333843]`}
                  disabled={!isEditing}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                ...
                <select
                  ...
                  className={`w-full border border-[#E0E2E7] p-3 rounded-lg ${isEditing ? 'bg-white' : 'bg-[#F9F9FC]'} text-[#333843]`}
                  disabled={!isEditing}
                >
        ...
        <div className="w-1/3 space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              ...
              <span className={`${status === 'Published' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600'} text-sm px-3 py-1 rounded-lg`}>
                {status}
              </span>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="text-sm text-gray-600">
              <p className="mb-2">ID: <span className="font-mono text-gray-800">{productId}</span></p>
              <p className="mb-2">Created: <span className="text-gray-800">April 4, 2025</span></p>
              <p>Last Updated: <span className="text-gray-800">April 5, 2025</span></p>
            </div>
          </div>
        </div>
      </div>
